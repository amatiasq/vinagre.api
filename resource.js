...
function resource(connection, name) {
	return {
		findAll: function() {
			...
			return connection.single('SELECT * FROM ' + name).then(function(results) { return results });
		},

		findById: function(id) {
			//console.log('Retrieving resource --[', name, ']-- id --[', id, ']--');
			var query = 'SELECT * FROM ' + name + ' WHERE ?';
			return connection.single(query, { id: id }).then(function(results) { return results });
		},
